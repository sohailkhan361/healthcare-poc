  ...
  return (
    <>
     <nav className='w-full h-[8ch] bg-neutral-50 flex items-center md:flex-row lg:px-32 md:px-16 sm:px-8 px-4 border-b border-neutral-200 z-50'>
        {/* Logo Section */}
        <Link to="/" className='text-2xl text-blue-600 font-bold mr-16 flex items-center'>
            <MdOutlineHealthAndSafety className='text-4xl'/> HealthCare+
        </Link>

        {/* Toggle */}
        <button onClick={handleClick} className="flex-1 md:hidden text-neutral-600 ease-in-out suration-300 flex items-center justify-end">
            {
                open ?
        ...
        {/* Nav items and btns */}
        <div className={`${open ? 'flex absolute top-16 left-[50%] translate-x-[-50%] w-[95%] h-auto md:relative': 'hidden'} 
        ...
        bg-neutral-50 md:border-none border border-neutral-200 md:shadow-none shadow-md rounded-md`}>
            ...
            {/* Login */}
            <Link to={"/login"} className="flex rounded-full bg-blue-600 hover:bg-blue-800 ease-in-out duration-300 p-2 md:items-center font-semibold cursor-pointer items-start gap-x-5 md:flex-row flex-col text-sm text-white">
                Login
            </Link>
